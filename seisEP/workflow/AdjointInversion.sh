...
do
    ...
    if [ ! -z "$sigma_x_min" ]; then
        smooth_x=$(( $smooth_x > $sigma_x_min ? $smooth_x : $sigma_x_min ))
    fi
    if [ ! -z "$sigma_z_min" ]; then
        smooth_z=$(( $smooth_z > $sigma_z_min ? $smooth_z : $sigma_z_min ))
    fi
    echo "          ***************************************"
    echo "          ** iteration $iter **"
...
cp -r $SUBMIT_RESULT/m_current $SUBMIT_RESULT/m_final
